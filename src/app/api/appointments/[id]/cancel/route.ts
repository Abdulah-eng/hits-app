...
export async function POST(
  ...
  try {
    ...
    // Only allow cancellation if user is involved in the appointment or is admin
    ...
    try {
      const notificationResponse = await fetch(`${request.nextUrl.origin}/api/notifications/appointment`, {
        ...
        body: JSON.stringify({
          ...
          custom_message: reason || `Cancelled by ${isClient ? 'client' : isSpecialist ? 'specialist' : 'admin'}`
        }),
      })
